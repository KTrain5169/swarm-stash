...
function cardSVG(card) {
    ...
    const gemRow = Array.from({ length: gems }, (_, i) => `<circle cx="${125 - (gems - 1) * 7 + i * 14}" cy="323" r="4" fill="${rc}" stroke="rgba(0,0,0,.4)"/>`).join('');
...
function cardEl(card, { qty, onClick, foil } = {}) {
    ...
    if (foil)
        el.insertAdjacentHTML('beforeend', '<div class="foil-badge">✦ FOIL</div>');
    if (qty > 1)
        el.insertAdjacentHTML('beforeend', `<div class="card-qty">×${qty}</div>`);
...
function closeZoom() {
    ...
    const done = (e) => {
        if (e && e.target !== overlay)
            return; // ignore bubbled child animations
...
function nav(view) {
    ...
    if (['binder', 'packs', 'swarm', 'arena', 'market', 'auction', 'ranks', 'trades', 'submit', 'modqueue'].includes(view))
...
document.addEventListener('click', (e) => {
    ...
    if (target === 'home')
        return nav('home'); // Home always goes home, logged in or not
    if (!state.me)
        return goToLogin(); // not logged in: send them to log in, not a no-op
    nav(target);
});
...
function updateNavOverflow() {
    ...
    requestAnimationFrame(() => {
        if (topbar.scrollWidth > topbar.clientWidth + 1)
            topbar.classList.add('nav-collapsed');
    });
}
...
document.addEventListener('click', (e) => {
    ...
    if (!panel.classList.contains('open'))
        return;
    if (panel.contains(e.target) || $('#nav-toggle').contains(e.target))
        return;
    closeNavPanel();
});
let navResizeT;
...
async function renderBinder() {
    ...
    $('#binder-filters').replaceChildren(...filters.map((f) => {
        ...
        if (f === 'all')
            ...
            if (inSeries.length && owned === inSeries.length)
    ...
    entries.forEach(([key, insts], i) => {
        ...
        sell.onclick = async () => {
            ...
            if (insts.length === 1 && !confirm(`Recycle your only ${label} for ⚡${value}?`))
                return;
            try {
        ...
        list.onclick = async () => {
            ...
            if (!price || price < 1)
                return toast('enter a valid price', true);
            try {
        ...
        pin.onclick = async () => {
            ...
            if (!pinned) {
                if (state.showcase.length >= 6)
                    return toast('showcase is full — unpin something first (6 max)', true);
...
function runPackOpening(pulls, onAllRevealed) {
    ...
    function dealStack() {
        ...
        const onReveal = () => {
            ...
            if (inst.foil)
                toast(`✦ FOIL ${card.name}! shiny shiny shiny`);
            if (card.rarity === 'legendary')
                toast(`🌟 LEGENDARY PULL: ${card.name}!!`);
            setTimeout(() => {
...
function fillPickGrid(side, insts) {
    ...
    grid.replaceChildren(...insts.map((inst) => {
        ...
        const el = cardEl(card, { foil: inst.foil, onClick: () => {
                if (picks[side].has(inst.instanceId))
                    picks[side].delete(inst.instanceId);
                else if (picks[side].size < 6)
                    picks[side].add(inst.instanceId);
                else
                    return toast('max 6 cards per side', true);
...
$('#trade-send').addEventListener('click', async () => {
    if (!picks.give.size || !picks.get.size)
        return toast('pick at least one card on each side', true);
    try {
        ...
        if (r.trade.status === 'accepted')
            toast(`🤝 ${state.member.user.name} accepted instantly!`);
        else if (r.trade.status === 'declined')
            toast(`😤 ${state.member.user.name} declined — offer more value`, true);
        else
...
async function renderTrades() {
    ...
    list.replaceChildren(...state.trades.map((t) => {
      ...
      <div class="trade-actions"></div>`;
        ...
        if (t.status === 'pending') {
            if (incoming) {
                actions.append(tradeBtn('Accept 🤝', 'btn-primary accept', () => actTrade(t.id, 'accept')), tradeBtn('Decline', 'btn-ghost', () => actTrade(t.id, 'decline')));
            }
            else {
...
async function loadBattles() {
    ...
    const needsMe = battles.filter((b) => (b.status === 'pending' && b.toId === state.me.id) ||
...
async function openBattleModal(mode) {
    ...
    $('#bteam-grid').replaceChildren(...state.collection.map((inst) => {
        ...
        const el = cardEl(card, { foil: inst.foil, onClick: () => {
                if (battlePick.has(inst.instanceId))
                    battlePick.delete(inst.instanceId);
                else if (battlePick.size < 3)
                    battlePick.add(inst.instanceId);
                else
                    return toast('a team is exactly 3 cards', true);
...
$('#battle-send').addEventListener('click', async () => {
    if (battlePick.size !== 3)
        return toast('pick exactly 3 cards', true);
    try {
        ...
        handleUnlocks(r);
        if (r.battle.status === 'declined') {
            toast('😤 challenge declined', true);
            nav('arena');
        }
        else if (r.battle.status === 'pending') {
            toast('⚔️ challenge sent!');
            nav('arena');
        }
        else
            openBattle(r.battle.id);
    }
    catch (err) {
        toast(err.message, true);
    }
    ...
    list.replaceChildren(...state.battles.map((b) => {
      ...
      <div class="trade-actions"></div>`;
        ...
        if (b.status === 'pending' && incoming) {
            actions.append(tradeBtn('Accept ⚔️', 'btn-primary accept', () => openBattleModal({ type: 'accept', battleId: b.id, name: other.name })), tradeBtn('Decline', 'btn-ghost', () => actBattle(b.id, 'decline')));
        }
...
async function openBattle(id) {
    ...
    try {
        ...
        if (battle.status === 'active')
            battlePoll = setInterval(refreshBattle, 2500);
    }
    catch (err) {
        toast(err.message, true);
    }
...
async function refreshBattle() {
    if (state.view !== 'battle' || !state.battle)
        return clearInterval(battlePoll);
    try {
...
function renderBattleScreen() {
    ...
    <div class="u-stats">${b.status === 'active' ? (myTurn ? '🔥 your turn!' : `waiting for ${esc(opp.name)}…`)
    ...
    const renderSide = (sel, uid, mine) => {
        ...
        if (!team) {
            el.innerHTML = '<p class="empty-note">team hidden until the challenge is accepted</p>';
            return;
        }
        ...
        team.forEach((f, i) => {
            ...
            if (canSwap) {
                ...
                if (foeActive) {
                    ...
                    fe.insertAdjacentHTML('beforeend', `<div class="swap-detail">deals ×${deal} · takes ×${take} vs ${esc(foeActive.name)}</div>`);
                }
            }
    ...
    if (b.status === 'active') {
        ...
        controls.append(atk, spc, tradeBtn('forfeit 🏳️', 'btn-ghost', () => {
            if (confirm('Forfeit this battle? The pot goes to your opponent.'))
                battleMove({ type: 'forfeit' });
        }));
        if (myTurn)
            controls.insertAdjacentHTML('beforeend', '<span class="hint">…or click a benched card to swap it in (uses your turn)</span>');
    }
...
function seriesMultClient(attacker, defender) {
    ...
    if (next(attacker) === defender)
        return 1.3;
    if (next(defender) === attacker)
        return 0.75;
    return 1;
}
...
function moveButton(f, moveIdx, target, myTurn) {
    ...
    if (type === 'heal') {
        ...
        if (mult > 1)
            parts.push('super effective!');
        else if (mult < 1)
            parts.push('resisted');
        if (mv.acc < 1)
            parts.push(`${Math.round(mv.acc * 100)}% to hit`);
        if (type === 'drain')
            parts.push('heals you half of it');
        if (type === 'break')
            parts.push(`then −20% DEF${target.defMod < 1 ? ` (now ${Math.round(target.defMod * 100)}%)` : ''}`);
    }
...
$('#challenge-btn').addEventListener('click', () => openBattleModal({ type: 'challenge', toId: state.member.user.id, name: state.member.user.name }));
// ─── Market ──────────────────────────────────────────────────────────────────
        ...
        btn.onclick = async () => {
            try {
                if (mine) {
                    ...
                    if (!confirm(`Buy ${card.name}${l.card.foil ? ' (foil)' : ''} from ${l.sellerName} for ⚡${l.price}?`))
                        return;
...
async function renderAuctions() {
    ...
    grid.replaceChildren(...auctions.map((a, i) => {
      ...
      <span class="auction-countdown" data-ends="${a.endsAt}"></span>`;
        ...
        if (mine) {
            ...
            btn.onclick = async () => {
                try {
                    await api(`/api/market/auctions/${a.id}/cancel`, { method: 'POST' });
            ...
            btn.onclick = async () => {
                ...
                if (!amount || amount < floor)
                    return toast(`bid must be at least ⚡${floor}`, true);
                try {
    ...
    const tick = () => {
        ...
        if (anyDue) {
            clearInterval(auctionTickTimer);
            renderAuctions();
        } // an auction just settled — refresh from the server
    };
    tick();
...
$('#start-auction-btn').addEventListener('click', async () => {
    ...
    $('#apick-grid').replaceChildren(...state.collection.map((inst) => {
        ...
        const el = cardEl(card, { foil: inst.foil, onClick: () => {
                ...
                if (!wasSelected) {
                    auctionPick.add(inst.instanceId);
...
$('#auction-start-send').addEventListener('click', async () => {
    if (auctionPick.size !== 1)
        return toast('pick a card to auction', true);
    ...
    if (!startingBid || startingBid < 1)
        return toast('enter a valid starting bid', true);
    if (!durationHours || durationHours < 1 || durationHours > 168)
        return toast('duration must be 1–168 hours', true);
    try {
...
function loadMemeFile(file) {
    ...
    if (!/^image\/(png|jpeg|gif|webp)$/.test(file.type))
        return toast('PNG, JPG, GIF or WEBP only', true);
    if (file.size > 5 * 1024 * 1024)
        return toast('max 5MB — compress that meme', true);
    ...
    reader.onload = () => {
        ...
        if (!$('#meme-name').value)
            $('#meme-name').value = file.name.replace(/\.[a-z0-9]+$/i, '').replace(/[-_]+/g, ' ').slice(0, 48);
    };
...
$('#submit-form').addEventListener('submit', async (e) => {
    e.preventDefault();
    if (!memeData)
        return toast('pick a meme first', true);
    const name = $('#meme-name').value.trim();
    if (!name)
        return toast('give your meme a name', true);
...
async function renderMySubmissions() {
    ...
    if (!memes.length) {
        holder.innerHTML = '<p class="empty-note">Nothing yet. Feed the swarm.</p>';
        return;
    }
...
async function renderQueue() {
    let memes;
    try {
        ({ memes } = await api('/api/memes/queue'));
    }
    catch (err) {
        toast(err.message, true);
        return nav('binder');
    ...
    list.replaceChildren(...memes.map((m) => {
      ...
      <div class="trade-actions"></div>`;
        const actions = $('.trade-actions', row);
        actions.append(tradeBtn('Approve ✓', 'btn-primary accept', async () => {
            try {
                await api(`/api/memes/${m.id}/approve`, { method: 'POST' });
                toast(`🎉 "${m.name}" minted as a card`);
                await refreshCatalog();
                renderQueue();
                refreshMe();
            }
            ...
            try {
                await api(`/api/memes/${m.id}/reject`, { method: 'POST' });
                toast('meme rejected');
                renderQueue();
                refreshMe();
            }
